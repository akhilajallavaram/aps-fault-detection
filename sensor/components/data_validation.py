...
class DataValidation:
    ...
    def drop_missing_values_column(self,df:pd.DataFrame,report_key_name:str)->Optional[pd.DataFrame]:
        ...
        ======================================================================================================
        returns a pandas DataFrame if atleast a single column is available after missing columns else drop
        """
        try:
            threshold = self.data_validation_config.missing_threshold
            null_report = df.isna().sum()/df.shape[0]
            #selcting column names which has null values
            logging.info(f"selecting column name with contains null values above to {threshold}")
            drop_column_names = null_report[null_report>threshold].index
            logging.info(f"Columns to drop: {list(drop_column_names)}")
            self.validation_error[report_key_name] = list(drop_column_names)
            #dropping the columns which contains null values greater than threshold
    ...
    def is_required_columns_exsists(self,base_df:pd.DataFrame,current_df:pd.DataFrame,report_key_name:str)->bool:
        try:
    ...
    def initiate_data_validation(self)->artifact_entity.DataValidationArtifact:
        try:
            ...
            train_df_column_status = self.is_required_columns_exsists(base_df=base_df,current_df=train_df,report_key_name="missing_values_within_train_dataset")
            logging.info(f"Is all required columns present in test df")
            test_df_column_status = self.is_required_columns_exsists(base_df=base_df,current_df=test_df,report_key_name="missing_values_within_test_dataset")
